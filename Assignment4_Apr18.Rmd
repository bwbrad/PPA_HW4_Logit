...
ga_recid <- read.csv("https://raw.githubusercontent.com/bwbrad/PPA_HW4_Logit/main/Data/NIJ_s_Recidivism_Challenge_Full_Dataset_20240409.csv",
                     na.strings = c(""," ","NA")) %>% 
  clean_names("snake") %>% 
  mutate(recidivism_within_3years = 
           case_when(recidivism_within_3years == "true" ~ 1,
                     recidivism_within_3years == "false" ~ 0),
         recid_dv_char = 
           case_when(recidivism_within_3years == 1 ~ "Reoffended in 3 years",
...
```{r create_partition}
ga_recid <- ga_recid %>% 
  mutate(prison_years_over3 = case_when(prison_years == "More than 3 years" ~ 1,
                                        prison_years %in% c("Less than 1 year", "Greater than 2 to 3 years") ~ 0),
         programs_over10 = case_when(program_attendances == "10 or more" ~ 1,
                                     TRUE ~ 0),
         programs_none = case_when(program_attendances == "0" ~ 1,
                                   TRUE ~ 0),
         older_than37 = case_when(age_at_release %in% c("38-42", "43-47", "48 or older") ~ 1,
                                  age_at_release %in% c("18-22", "23-27", "23-27", "28-32", "33-37") ~ 0),
         some_college = case_when(education_level == "At least some college" ~ 1,
                                  education_level %in% c("High School Diploma", "Less than HS diploma") ~ 0),
         high_supervision = ifelse(supervision_level_first == "High", 1, 0),
         offense_property = case_when(prison_offense == "Property" ~ 1,
                                      TRUE ~ 0),
         offense_violent_sex = case_when(prison_offense == "Violent/Sex" ~ 1,
                                         TRUE ~ 0),
         one_dependent = case_when(dependents == "1" ~ 1,
                                   TRUE ~ 0),
      
    )

ga_recid <- ga_recid %>% 
  filter(!is.na(supervision_level_first))
...
```{r}
recid_model_1 <- glm(recidivism_within_3years ~ .,
                     data = recid_train %>%
                       dplyr::select(prison_years_over3, programs_over10, older_than37, some_college,
                                     high_supervision, offense_property, offense_violent_sex, one_dependent,
                                     recidivism_within_3years),
                     family="binomial" (link="logit"))

summary(recid_model_1)

pR2(recid_model_1)

recid_model_2 <- glm(recidivism_within_3years ~ some_college + offense_property +
                       offense_violent_sex + percent_days_employed + jobs_per_year +
                       prison_years_over3 + older_than37 + programs_over10 + 
                       high_supervision,
                     data = ga_recid,
                     family="binomial" (link="logit"))

summary(recid_model_2)
...
recid_model_3 <- glm(recidivism_within_3years ~ some_college + offense_property +
                       ...
                       prison_years_over3 + older_than37 + programs_over10 +
                       high_supervision + supervision_risk_score_first + one_dependent +
                       prior_arrest_episodes_dv_charges + prior_revocations_parole +
                       prior_revocations_probation,
                     data = ga_recid,
                     family="binomial" (link="logit")) 

summary(recid_model_3)

pR2(recid_model_3)

```

- recid_model_2: 65% accuracy, more false positives than false negatives
- recid_model_3: 69% accuracy, still more false positives

```{r}
test_probs <- data.frame(outcome = as.factor(recid_test$recidivism_within_3years),
                         probs = predict(recid_model_3, recid_test, type= "response")) %>%
  mutate(pred_outcome = as.factor(ifelse(probs > 0.5 , 1, 0)))

caret::confusionMatrix(test_probs$pred_outcome, test_probs$outcome, 
                       positive = "1")
```
...
auc(test_probs$outcome, test_probs$probs)


ggplot(test_probs, aes(d = as.numeric(outcome), m = probs)) +
...
### Confusion Matrix
The confusion matrix below highlights our model's relative sensitivity and specificity. It shows a True Positive value of 1688, meaning 1688 entries are correctly identified as positive. In this case, that means our model correctly predicts 1688 people who experience recidivism within three years. The False Positive value is 1742, meaning 1742 people are incorrectly predicted to experience recidivism within three years. The False Negative value of 992 represents the number of people who will experience recidivism in the next three years that our model failed to predict. The True Negative value of 3638 represents the number of people our model correctly predicted to stay out of the prison system within three years. 

The sensitivity, or recall, of our model is 0.63, meaning it correctly identifies 63% of people who will experience recidivism within three years. The specificity of our model is 0.68, meaning a true negative rate of 68%, which is the rate at which those who will not experience recidivism in three years are correctly identified.

A higher specificity rate means our model more accurately predicts those who will not reoffend. This is useful for implementing legal practices that help protect these people from further hardship within the prison system. 

#### Can someone check what I wrote^^ I calculated the specificity and sensitivity rates, but they aren't the same as what's printed below...not sure how i did that incorrectly.
```{r thresholds}
test_probs <- 
  test_probs %>%
  mutate(pred_outcome  = as.factor(ifelse(test_probs$probs > 0.5 , 1, 0)))
```
...
ggplot(subset(test_probs, pred_outcome != "NA"), aes(x = probs, fill = as.factor(outcome))) + 
